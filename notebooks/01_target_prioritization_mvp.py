...
def run_graphql_query(query: str, variables: dict) -> dict:
    ...
    Prints the API error message if the query fails.
    """
...
def search_disease(query: str, size: int = 10) -> pd.DataFrame:
    ...
    Example: search_disease("melanoma")
    """
    graphql_query = """
    query SearchDisease($queryString: String!, $index: Int!, $size: Int!, $entityNames: [String!]!) {
      search(
...
def get_associated_targets(disease_id: str, size: int = 100) -> pd.DataFrame:
    ...
    Example disease ID: EFO_0000756 for melanoma-related terms, depending on search result.
    """
    graphql_query = """
    query AssociatedTargets($diseaseId: String!, $index: Int!, $size: Int!) {
